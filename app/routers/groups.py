...
async def list_groups(
    ...
    q = (
        select(Group)
        .options(joinedload(Group.teacher), selectinload(Group.group_students))
    )
    if course_id:
...
async def frozen_students(db: AsyncSession = Depends(get_db), _=Depends(get_current_user)):
    result = await db.execute(
...
async def group_students(
    ...
    q = select(User).join(GroupStudent, GroupStudent.student_id == User.id).where(GroupStudent.group_id == group_id)
